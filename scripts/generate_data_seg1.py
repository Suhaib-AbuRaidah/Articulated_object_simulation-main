...
def collect_observations(sim, args):
    ...
    start_state = np.random.uniform(-3,3)
    sim.set_joint_state(0, -np.pi/4)
    sim.set_joint_state(1, np.pi/6)
    joint_index = 0
    joint_type = 0
    ...
    start_state=np.random.uniform(-3,3)
    end_state=np.random.uniform(-1,1)
    sim.set_joint_state(0, -np.pi/4)
    sim.set_joint_state(1, np.pi/6)
    if args.is_syn:
    ...
    sim.set_joint_state(0, np.pi/8)
    sim.set_joint_state(1, -np.pi/4)
