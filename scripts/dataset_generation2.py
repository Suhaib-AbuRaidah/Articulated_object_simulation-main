...
SEGMENTATION_COLORS_RGBA = [
    [channel / 255.0 for channel in hex_to_rgb(color)] + [1.0]
...
def set_link_visual_colors(sim: Any, links: List[int], colors: List[List[float]]) -> None:
    for color_idx, link_id in enumerate(links):
        sim.world.p.changeVisualShape(
            sim.object.uid,
            int(link_id),
            rgbaColor=colors[color_idx % len(colors)],
        )


def render_segmented_images(sim: Any, n: int, links: List[int]) -> List[np.ndarray]:
    set_link_visual_colors(sim, links, SEGMENTATION_COLORS_RGBA)

    rgb_imgs = []
    for extrinsic in render_viewpoint_extrinsics(sim, n):
        rgb_img, _ = sim.camera.render(extrinsic)
