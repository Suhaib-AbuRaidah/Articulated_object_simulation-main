...
class ArticulatedObjectManipulationSim(object):
	...
	def reset(self, index=None, canonical=False):
		...
		self.draw_workspace()
		#log_id = pybullet.startStateLogging(pybullet.STATE_LOGGING_VIDEO_MP4, "simulation_video.mp4")
		if self.gui:
...
#         self.place_table(table_height)
	...
	def acquire_segmented_pcs(self, n, links, N=None):
		...
		Returns:
			depth_imgs       : list of depth images
			rgb_imgs         : list of rgb images
			pcs_per_link     : dict: link_id → point cloud (N_l × 3)
			masks_per_link   : dict: link_id → mask (N_l)
			mesh_pose_dict   : mesh poses
		"""
		...
		origin = Transform(
			Rotation.identity(),
			np.r_[self.size / 2, self.size / 2, self.size / 2]
		)
		r = 1.2 * self.size
		extrinsics = []
		...
		if self.dense_photo:
			for offset, theta in zip([0, 1, 2], [np.pi/8, np.pi/4, np.pi/2]):
				N = N if N else n
				phi_list = 2.0 * np.pi * np.arange(n) / N + offset * 2.0 * np.pi / (N * 3)
				extrinsics += [
					camera_on_sphere(origin, r, theta, phi)
					for phi in phi_list
				]
		else:
			theta = np.pi / 4.0
		...
		# -----------------------------
		# Extract per-link clouds & masks from TSDF
		# -----------------------------
		pcs_per_link = {}
		masks_per_link = {}
